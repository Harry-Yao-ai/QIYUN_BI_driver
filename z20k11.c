...
/*                                                                        	  */
/*============================================================================*/ 
/* UPDATE HISTORY - QIYUN                                                     */
/*                                                                            */
/* 1.00 Harry Yao 14May2021                                                   */
/******************************************************************************/
/******************************************************************************/
...
#ifndef TARGET_NVM
#ifndef TARGET_HTOL
#error "Select either TARGET_NVM or TARGET_HTOL in driver source .c file"
#endif
#endif
...
#ifdef TARGET_NVM
#ifdef TARGET_HTOL
#error "Select only one TARGET_NVM or TARGET_HTOL in driver source .c file"
#endif
#endif
...
/******************************************************************************/
	...
	VdutsPowerDownOrder(4,3,2,1,5,100000);	/* orderly shut-down on driver supply event w/100ms delay */
											/* VPULL first, then VDUT for LDO, VDUT1 for VDD is the last
...
// Initialize resources, not used until after VddRamp() selects actual operating conditions
	...
	if(DRIVER_TYPE == CC3E)
	{
		//just used for initial, the voltage will be configured on VddRamp()
	...
	else if(DRIVER_TYPE == CC3)
	{
 		...
 		AnOut(9000); 						/* Set Anout to 9V to power up eFuse circuit */
		SET_VIO_VDUT1;						/* VIO  follows VDUT1 (or VDUT4) */
		SET_VCLK_VDUT1;						/* VCLK follows VDUT1 (or VDUT2) */
...
/********************************************************************************************/
	StartButton =QCHECK;
//	StartButton = BIN2_CHECK;				/*for debug only */
/***************************************************************************************/
...
//				ShutDown();										/* just for debug */
				...
				{	
				DisplayBadDutsOnce();  
				//StartButton =  BURN_IN; 						/* just for debug */
				}
...
// 				SWDInterimWrite(FLASH_TEST_REG,FlashAccessKey1);
// 				SWDInterimWrite(FLASH_TEST_REG,FlashAccessKey2);
...
//there is only three status add for QIYUN
//				DisableDutData(9);
...
void ShutDown(void)								/* power off all resources in an orderly fashion */
{
	...
	WaitSeconds(4);								/* add delay to wait the power-down */
}
...
// Now set the AHB_TAR to the address of the 1st word in status, read using AHB_DRW
	SWDWriteReg(APACC,AHB_TAR,ADC_Ver_Loc);			/* Set target location in TAR for APACC memory access */
...
// Actual AHB_DRW data is shifted out on command following read request - read again to capture true results
	...
	{
		if ((QCHKResult[i] !=  ADC_Ver_ID))			/* Protection and security should be disabled */
		{
...
// Adjust ECB limits based on number of populated sockets. Fraction of current limit coincides with fraction of populated sockets
	...
	PartialECB1 = (ECB1_RUN * (DutNumbers[0]-countFail) / DutNumbers[0]) + 800; 	//for debug
	PartialECB2 = (ECB2_RUN * (DutNumbers[0]-countFail) / DutNumbers[0]) + 800;		//for debug
...
//ECC disable
	SWDInterimWrite(0x40021104,0x273048FD);
// 	SWDInterimWrite(0x40021160,0x00010000);
...
// Set the AHB_TAR to the address of the 1st word in status, read using AHB_DRW
	...
	SWDWriteReg(APACC,AHB_TAR,HTOLStatusAddr);		/* Selects AHB-AP TAR for APACC access */	
#endif
...
// For CC3/E compatibility, don't declare buffer output levels until power supplies in normal operating range
				...
				if(DRIVER_TYPE == CC3E)
				{
				//if VDD > 5.5V, SetPsBuf1(voltage[0],DUTCODE) will have no effect
//					SetPsBuf1(voltage[0],DUTCODE);		/* Driver tracks with VDUT1 */
//					SetPsBuf2(voltage[0],DUTCODE);		/* Driver tracks with VDUT1 */
					SetPsBuf1(voltage[0],OSSET);		/* Driver tracks with VDUT1 */
					SetPsBuf2(voltage[0],OSSET);		/* Driver tracks with VDUT1 */
...
// TestModeFlag determines/selects Regulator Bypass. Always be in Functional Test Mode (set in SWDDownloadS19)
				...
				{
					SWDWriteReg(APACC,MDM_TR,Access_Key);
					SWDWriteReg(APACC,MDM_TR,FuncTest_High);	/* power up DAC */
					SWDWriteReg(APACC,MDM_IDR,FuncTest_Low);
...
// To facilitate reads through ARM debug port, reset, clear PMC bit in Test Reg, DAC control, assert core hold.
				...
				SWDWriteReg(APACC,AHB_CSW,0x23000012);
			#ifdef DEBUG
				SWDReadReg(APACC,AHB_CSW, ReadResult);			/* first read posts data to shift chain */
				SWDReadReg(APACC,AHB_CSW, ReadResult);			/* second read shifts data out for capture */
			#endif	
			// Below is the sequence for QIYUN
			// re halt core will generate core reset
				SWDWriteReg(APACC,AHB_TAR,0xe000ed0c);			/* Set target location in TAR for APACC memory access */
			#ifdef DEBUG
				SWDReadReg(APACC,AHB_TAR, ReadResult);			/* first read posts data to shift chain */
				SWDReadReg(APACC,AHB_TAR, ReadResult);			/* second read shifts data out for capture */
			#endif
				SWDWriteReg(APACC,AHB_DRW,0x05fa0004);		/* check status from previous write, capture results */
			#ifdef DEBUG
			...
			#endif
			// write dfsr[0] bit 1 to halt cpu
				SWDWriteReg(APACC,AHB_TAR,0xe000edf0);			/* Set target location in TAR for APACC memory access */
			#ifdef DEBUG
				SWDReadReg(APACC,AHB_TAR, ReadResult);			/* first read posts data to shift chain */
				SWDReadReg(APACC,AHB_TAR, ReadResult);			/* second read shifts data out for capture */
			#endif
				SWDWriteReg(APACC,AHB_DRW,0xa05f0003);			/* check status from previous write, capture results */
			#ifdef DEBUG
				SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
				SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
			#endif
			...
			#endif
//				SWDWriteReg(APACC,AHB_CSW,0x23000012);
				SWD_AHB_CFG(BURN_IN,data[k].TestModeFlag);
			#ifdef DEBUG	
//				SWDWriteReg(APACC,AHB_TAR,TempSenseLoc);
				SWDWriteReg(APACC,AHB_TAR,0x1fffc000);
				SWDReadReg(APACC,AHB_TAR, ReadResult);
				SWDReadReg(APACC,AHB_TAR, ReadResult);
				SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
				SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
				SWDWriteReg(APACC,AHB_TAR,0x1fffc004);
				SWDReadReg(APACC,AHB_TAR, ReadResult);
				SWDReadReg(APACC,AHB_TAR, ReadResult);
				SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
				SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
				SWDWriteReg(APACC,AHB_TAR,0x1fffc070);
				SWDReadReg(APACC,AHB_TAR, ReadResult);
				SWDReadReg(APACC,AHB_TAR, ReadResult);
				SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
				SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
...
// power off the device on the end of the project loop to avoid over-stress
	Vdut3Off();							/* turn off during ramp down */
	Vdut4Off();							/* turn off during ramp down */
	WaitSeconds(1);						/* Delay 1s while signals decay */
	
	SetVdut(2,1200);					/*	try to give clean POR deassertion */
	SetVdut(1,1200);					/*	try to give clean POR deassertion */
	WaitSeconds(1);
...
/***************************************************************************************/
#ifdef CLEAR_NVM_SECURITY
//this block is used for security earse for some products, can be ignored here
...
//	if (Button == BURN_IN)
//	if ((Button == BURN_IN)||(Button == QCHECK))
//	{
//		if (TestMode == 1)
...
//			SWDWriteReg(APACC,MDM_TR,FuncTest);	/* 0: do not require DAC */
		SWDWriteReg(APACC,MDM_TR,Access_Key);
#ifdef	DEBUG
		SWDReadReg(APACC,MDM_TR,ReadResult);
#endif
		SWDWriteReg(APACC,MDM_IDR,FuncTest_High);
#ifdef	DEBUG
		SWDReadReg(APACC,MDM_IDR,ReadResult);
#endif
		SWDWriteReg(APACC,MDM_TR,FuncTest_Low);
#ifdef	DEBUG
		SWDReadReg(APACC,MDM_TR,ReadResult);
#endif
...
//	SWDWriteReg(APACC,MDM_CR,0x00000010);				/* clear system reset request (bit3), maintain CM4 Core Hold reset */
														/* this configuration was removed temporily, because it will cause QIYUN Qcheck fail */
...
//to check if enter test mode successfully
#ifdef DEBUG
	
	if(Button == BURN_IN)
	{
		SWDWriteReg(APACC,AHB_TAR,0x4007D02c);			/* Set target location in TAR for APACC memory access */
		SWDReadReg(APACC,AHB_TAR, ReadResult);			/* first read posts data to shift chain */
		SWDReadReg(APACC,AHB_TAR, ReadResult);			/* second read shifts data out for capture */
		SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
		SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */		
...
/***************************************************************************************/
void SWDInterimWrite(UINT32 ADDR, UINT32 DUTData)
{
...
//	SWDWriteReg(APACC,AHB_CSW,0x23000002);				/* Selects AHB-AP CSW for APACC access */
//	SWD_AHB_CFG(QCHECK,0);								/* set up for polling */
	SWDWriteReg(APACC,AHB_TAR,ADDR);					/* Selects AHB-AP TAR for APACC access */
	SWDWriteReg(APACC,AHB_DRW,DUTData);					/* Write Data to AHB_AP DRW */
//	SWDReadReg(APACC, AHB_DRW, ReadResult);				/* first read posts data to shift chain */
//	SWDReadReg(APACC, AHB_DRW, ReadResult);				/* second read shifts data out for capture */	
}	
...
// Algorithm insures a non-zero value is always specified for the delay
	...
	UINT32	TimeUsed,TimeRemaining,current1;
	current1 = GetCurrent(1);
	current1 = GetCurrent(2);
	current1 = GetCurrent(5);
...
void 	PolledDelay_BIST(UINT32 DelayTime)
{
...
// Algorithm insures a non-zero value is always specified for the delay
	...
	while ( (StartButton != BIN2_CHECK) && (DelayTime > TimeUsed) )
	{
		...
		{
			RESET_LOW;
			RESET_HIGH;									/* for QIYUN, need to toggle reset pin to restart BIST */
			WaitSeconds(TimeSegment);					/* check for Bin2 button every nn seconds */
			TimeUsed = TimeUsed + TimeSegment;
		}
		else
...
// If 1st time we are powerng up, do not set VDUT1 on then off again . . .
	...
	if (PreviousVdd != 0000)
	{
	//for QIYUN, to make Vdut1 and Vdut2 power off at the same time
	SetVdut(2,1200);					/*	try to give clean POR deassertion */
	SetVdut(1,1200);					/*	try to give clean POR deassertion */
	WaitSeconds(1);
...
// Select 32-bit register access, no auto-increment to enable register write
	SWDWriteReg(APACC,AHB_CSW,0x23000002);			/* Selects AHB-AP CSW for APACC access, DO NOT DISTURB PROT BITS */
// Specify Target Address register for PMU_USER_LOCK register
	SWDWriteReg(APACC,AHB_TAR,PMC_LOCK);
// Write PMU Unlock Key in Lock Register to Unlock PMU Register
	SWDWriteReg(APACC,AHB_DRW,PMC_USER_UNLOCK);
#ifdef DEBUG
	SWDWriteReg(APACC,AHB_TAR,PMC_LOCK);
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
#endif
	//WaitMicroSecondsInt(500);
// Select 32-bit register access, no auto-increment to enable register write
	SWDWriteReg(APACC,AHB_CSW,0x23000002);			/* Selects AHB-AP CSW for APACC access, DO NOT DISTURB PROT BITS */
// Specify Target Address Register for LVR control
	SWDWriteReg(APACC,AHB_TAR,PMC_VDD5V_CSR);		/* Selects AHB-AP TAR for APACC access - Low Voltage Detect Status And Control 1 register */
...
#ifdef DEBUG
	SWDWriteReg(APACC,AHB_TAR,PMC_VDD5V_CSR);
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
#endif
//	DisableDutData(1);								/* prevent host from polling prior to data initialization */
...
//  intialize the SRAM for QIYUN
	WaitMicroSeconds(1000);
	SRAM_init();
	WaitSeconds(1);									/* for debug */
// Set up data transfer - AddrInc[5:4]=01 for autoincrement, Size[2:0]=010 for 32-bit writes
...
//to check if the data was downlaod to SRAM successfully
//#ifdef DEBUG
//			SWDWriteReg(APACC,AHB_TAR,address);
//			SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
//			SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
//			address += 4;
//#endif
...
//    out at least 8 more clock rising edges, before stopping the clock.
	...
	WaitMicroSeconds(1000);					/* Default delay 10mS after completing operation */
...
// Set up data transfer - AddrInc[5:4]=00 for NO autoincrement, Size[2:0]=010 for 32-bit writes
	SWDWriteReg(APACC,AHB_CSW,0x23000002);		/* Selects AHB-AP CSW for APACC access */
// clear the hardfault irq
	SWDWriteReg(APACC,AHB_TAR,0xe000ed0c);			/* Set target location in TAR for APACC memory access */
#ifdef DEBUG
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* second read shifts data out for capture */
#endif
	SWDWriteReg(APACC,AHB_DRW,0x05fa0002);		/* check status from previous write, capture results */
#ifdef DEBUG
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
#endif
// write dfsr[0] bit 1 to release cpu
	SWDWriteReg(APACC,AHB_TAR,0xe000edf0);			/* Set target location in TAR for APACC memory access */
#ifdef DEBUG
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* second read shifts data out for capture */
#endif
	SWDWriteReg(APACC,AHB_DRW,0xa05f0000);			/* check status from previous write, capture results */
#ifdef DEBUG
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
#endif
...
#endif
// R15(PC) is selected
	SWDWriteReg(APACC,AHB_TAR,0xe000edf4);			/* Set target location in TAR for APACC memory access */
#ifdef DEBUG
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* second read shifts data out for capture */
#endif
	SWDWriteReg(APACC,AHB_DRW,0x000f);				/* check status from previous write, capture results */
#ifdef DEBUG		
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
#endif
// to wait status ready
	SWDWriteReg(APACC,AHB_TAR,0xe000edf0);			/* Set target location in TAR for APACC memory access */
#ifdef DEBUG
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* second read shifts data out for capture */
	...
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
// to monitor the current PC
	SWDWriteReg(APACC,AHB_TAR,0xe000edf8);			/* Set target location in TAR for APACC memory access */
#ifdef DEBUG
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* second read shifts data out for capture */
#endif
	...
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
// to write the PC where you want to run
	SWDWriteReg(APACC,AHB_TAR,0xe000edf8);			/* Set target location in TAR for APACC memory access */
#ifdef DEBUG
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* second read shifts data out for capture */
#endif
	SWDWriteReg(APACC,AHB_DRW,PC_loc);				/* check status from previous write, capture results */
#ifdef DEBUG		
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
#endif
...
// R15(PC) is selected
	...
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* second read shifts data out for capture */
#endif
	SWDWriteReg(APACC,AHB_DRW,0x1000f);				/* check status from previous write, capture results */
#ifdef DEBUG		
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
#endif	
// to write the location where you want to use as the stack
	SWDWriteReg(APACC,AHB_TAR,0xe000edf8);			/* Set target location in TAR for APACC memory access */
#ifdef DEBUG
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* second read shifts data out for capture */
#endif
	SWDWriteReg(APACC,AHB_DRW,Stack_loc);			/* check status from previous write, capture results */
#ifdef DEBUG		
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
#endif
...
// MSP is selected
	...
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* second read shifts data out for capture */
#endif
	SWDWriteReg(APACC,AHB_DRW,0x10011);				/* check status from previous write, capture results */
#ifdef DEBUG		
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
#endif
// to write the data for xPSR
	SWDWriteReg(APACC,AHB_TAR,0xe000edf8);			/* Set target location in TAR for APACC memory access */
#ifdef DEBUG
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* second read shifts data out for capture */
#endif
	SWDWriteReg(APACC,AHB_DRW,0xffffffff);			/* check status from previous write, capture results */
#ifdef DEBUG		
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
#endif
...
// xSPR is selected
	...
	SWDReadReg(APACC,AHB_TAR, ReadResult);			/* second read shifts data out for capture */
#endif
	SWDWriteReg(APACC,AHB_DRW,0x10010);				/* check status from previous write, capture results */
#ifdef DEBUG		
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* first read posts data to shift chain */
	SWDReadReg(APACC,AHB_DRW, ReadResult);			/* second read shifts data out for capture */
#endif
...
//		SWDWriteReg(APACC, MDM_TR, BISCAN);
		SWDWriteReg(APACC,MDM_TR,Access_Key);
		SWDWriteReg(APACC,MDM_IDR,BISCAN_High);
#ifdef	DEBUG
		SWDReadReg(APACC,MDM_IDR,ReadResult);
		SWDReadReg(APACC,MDM_IDR,ReadResult);
#endif
		SWDWriteReg(APACC,MDM_TR,BISCAN_Low);
#ifdef	DEBUG
		SWDReadReg(APACC,MDM_TR,ReadResult);
		SWDReadReg(APACC,MDM_TR,ReadResult);
#endif
	...
	SWDReadReg(APACC, MDM_IDR, SCANMode);			/* check status from previous write, capture results */
	SWDReadReg(APACC, MDM_IDR, SCANMode);			/* data is posted on first read, captured on second  */
...
*/
	...
	SetIO ( 24, HSVG, 1, INV_OFF);					/* OP.24 drives XTAL/EDT_CLK bank 1  */
	SetIO ( 25, HSVG, 1, INV_OFF);					/* OP.25 drives XTAL/EDT_CLK bank 2  */
...
// Restore control of driver resources to output commands
	...
	SetIO ( 24, IO, 1, INV_OFF);						/* OP.4 drives XTAL/EDT_CLK bank 1  */
	SetIO ( 25, IO, 1, INV_OFF);						/* OP.5 drives XTAL/EDT_CLK bank 2  */
...
//		TOGGLE_EDT_CLOCK(1);
		for (dut = 0; dut < SCANNumbers[0]; dut++)		
		{
			...
			ReadResult[dut] = (DUT_INPUT == 1) ? (ReadResult[dut] | DataMask) : (ReadResult[dut] & ~DataMask);
		}
	}
...
// LSB                15937159
	...
	*(data_ptr++) = 0x00003030;						/* EDT_CLK clock pulse on XTAL uses OP.24/25 */
	*(data_ptr++) = 0x00003000;	
	*(data_ptr++) = 0x02003000;	
	*(data_ptr++) = 0x00003000;	
	*(data_ptr++) = 0x00003030;	
...
void SWD_RAMBIST(struct HSVGModule *module, UINT32 *BISTData, UINT32 *BISTMode)
{
	...
	else
	{
		...
		SWDReadReg(APACC,MDM_TR,ReadResult);
#endif
		SWDWriteReg(APACC,MDM_IDR,MBIST_High);
#ifdef	DEBUG
		SWDReadReg(APACC,MDM_IDR,ReadResult);
		SWDReadReg(APACC,MDM_IDR,ReadResult);
#endif
		SWDWriteReg(APACC,MDM_TR,MBIST_Low);
#ifdef	DEBUG
		SWDReadReg(APACC,MDM_TR,ReadResult);
		SWDReadReg(APACC,MDM_TR,ReadResult);
#endif
	...
	SWDReadReg(APACC,MDM_IDR,BISTMode);				/* check status from previous write, capture results */
	SWDReadReg(APACC,MDM_IDR,BISTMode);				/* data is posted on first read, captured on second  */
...
//	WaitSeconds(module->seconds - DisplayTime);
	...
	TimeToRun = module->seconds - DisplayTime;			
//	ShutDown();										/* only for debug */
	PolledDelay_BIST(TimeToRun);							/* fragment delay to check for Bin2 request */
...
// SRAM Init
// For QIYUN, it requires SRAM init before you want to use it
void SRAM_init(void)
{
	int SRAM_addr;
...
// Select 32-bit register access, no auto-increment to enable register write
	...
	while(SRAM_addr < SRAM_END)	
	{
		...
		SWDWriteReg(APACC,AHB_TAR,SRAM_addr);			/* Selects AHB-AP TAR for APACC access */
		
		SWDWriteReg(APACC,AHB_DRW,0);	/* Selects AHB-AP DRW for APACC access */
